...
class NewChatSheet extends StatefulWidget {
  ...
  final Function(Chat, bool) onCreate;
  ...
  final Future<bool> Function(List<UserInfo> users) confirmation;
...
class _NewChatSheetState extends State<NewChatSheet> {
    ...
    _controller = TextEditingController()
      ..addListener((){
        ...
        if(redex.isNotEmpty){
          ...
          searchBloc.add(LoadThis<List<UserInfo>>(() async {
            //On defualt loads the users the login user is following
            List<String> followingUsers = (await FollowApi.getFollowingByUserId(PollarStoreBloc().loggedInUserID)).map<String>((f) => f.recipientId).toList();
            List<UserInfo> baseUserList = [...users, ...(await UserInfoApi.getBatchUserInfoById(followingUsers))];
  ...
  void onConfirm(){
    Chat newChat = ChatBloc().findUserChat(users.map<String>((u) => u.id).toList());

    //Call the call back function
    if(widget.onCreate != null){
      widget.onCreate(newChat, ChatBloc().state[newChat.id] == null);}
  }
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      //       bool confirm = widget.confirmation != null ? await widget.confirmation(users) : true; 
                      //       if(confirm){
                      //         NavStack().pop();
                      //         onConfirm();
